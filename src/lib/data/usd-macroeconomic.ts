...
async function fetchRealEconomicData(): Promise<Record<string, any>> {
	...
	try {
		...
		const realData = await batchFetchFREDData(seriesConfigs);

		// Cache the results
		cachedData = realData;
		lastFetchTime = now;

		console.log(`Successfully fetched ${Object.keys(realData).length} USD economic indicators`);
		return realData;
